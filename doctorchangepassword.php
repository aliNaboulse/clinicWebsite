...
if (isset($_POST['change'])) {
	...
	if(!empty($passw1) &&!empty($passw2)){
	    ...
	    if($passw1==$passw2){
            ...
            $query="update doctors set password= unhex(sha2('$passw1',256)) where userID=".$_SESSION['userID'];

            mysqli_query($conn,$query);

    		if (mysqli_affected_rows($conn)<=0){

    		    $result="Password didn't changed.";
...
<form action="doctorchangepassword.php" method=post>
